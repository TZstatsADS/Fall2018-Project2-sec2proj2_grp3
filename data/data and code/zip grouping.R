...
rownames(zip.prop) <- unique(rest$ZIP)
colnames(zip.prop) <- names(table(rest$TYPE))
head(zip.prop)
...
row.names(zip.rate) <- unique(rest$ZIP)
colnames(zip.rate) <- "Yelp star"
head(zip.rate)
...
for(i in 1:length(unique(rest$ZIP))){
  zip.class[i] <- ifelse(is.na(zip.income[i]),NA, ifelse(as.numeric(as.character(zip.income))[i]<=as.numeric(income.quartiles[2]), "Poor", ifelse(as.numeric(as.character(zip.income))[i]<=as.numeric(income.quartiles[3]), "Below Median", ifelse(as.numeric(as.character(zip.income))[i]<=as.numeric(income.quartiles[4]),"Above median", "Rich"))))
}

zip.class <- cbind(as.numeric(as.character(zip.income)),as.numeric(as.character(zip.class)))
head(zip.class)
...
demog <- read.csv("age.sex.csv", header = T)
...
rownames(zip.age) <- unique(rest$ZIP)
colnames(zip.age) <- names(demog[,4:11])
head(zip.age)
