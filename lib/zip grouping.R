...
##############################################################
# zip.area  - type of restaurant per person in each zip code #
##############################################################

area <- read.csv("density.csv")
area <- area[,c(1,3)]
area.ind <- rep(NA, length(unique(rest$ZIP)))
for(i in 1:length(unique(rest$ZIP))){
  area.ind[i] <- ifelse(length(which(area$Zip.ZCTA==unique(rest$ZIP)[i]))>0,which(area$Zip.ZCTA==unique(rest$ZIP)[i]),NA)
}

zip.area <- area$Land.Sq.Mi[area.ind]
zip.area <- matrix(c(unique(rest$ZIP),zip.area),196,2)
colnames(zip.area) <- c("ZIP","SQM")
head(zip.area)

#####################################################
# zip.density  - density of people in each zip code #
#####################################################

zip.density <- zip.pop/zip.area[,2]
zip.density <- matrix(c(unique(rest$ZIP),zip.density),196,2)
colnames(zip.density) <- c("ZIP","density")
head(zip.density)
